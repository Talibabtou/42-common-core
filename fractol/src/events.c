...
/*   Updated: 2024/02/07 15:52:24 by gdumas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	zoom(t_fractol *f, double zoom)
{
	...
	center_i = (f->max_i + f->min_i) / 2.0;
	half_range_r = (f->max_r - f->min_r) / 2.0;
	half_range_i = (f->max_i - f->min_i) / 2.0;

	f->min_r = center_r - half_range_r * zoom;
	f->max_r = center_r + half_range_r * zoom;
	f->min_i = center_i - half_range_i * zoom;
	f->max_i = center_i + half_range_i * zoom;
}
